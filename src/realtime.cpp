...
void Realtime::setUpHouseMesh(const glm::mat4& ctm, SceneMaterial mat, const char* path) {
    ...
    //     glm::vec3 vertex(data[i], data[i + 1], data[i + 2]);
...
// bool Realtime::checkCollisions(){
//     if (m_shapeDataList.size() > 0){
//         glm::vec3 balloonboxMin = m_shapeDataList[0].boundingBoxMin;
//         glm::vec3 balloonboxMax = m_shapeDataList[0].boundingBoxMax;
//         glm::vec3 houseboxMin = m_shapeDataList[1].boundingBoxMin;
//         glm::vec3 houseboxMax = m_shapeDataList[1].boundingBoxMax;

//         // if (balloonboxMax.x < houseboxMin.x || balloonboxMin.x < houseboxMax.x) return false;
//         // if (balloonboxMax.y < houseboxMin.y || balloonboxMin.y < houseboxMax.y) return false;
//         // if (balloonboxMax.z < houseboxMin.z || balloonboxMin.z < houseboxMax.z) return false;

//         if (balloonboxMin.x < houseboxMax.x) return true;
//         if (balloonboxMin.y < houseboxMax.y) return true;
//         if (balloonboxMin.z < houseboxMax.z) return true;
//         return false;
